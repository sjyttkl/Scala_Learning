...
package scala3 {
  ...
  class User { // 在com.sjyttkl包下创建个 User类
    def sayHello(): Unit = {
      //想使用 com.sjyttkl.scala2包下的 Monster
      import scala3.scala4.Monster
      val monster = new Monster()
    }
  }


    package scala4 { // 创建包 com.sjyttkl.scala4
      class User { // 在com.sjyttkl.scala4 包下创建个 User类
     }
  ...
  //2. package object scala 表示创建一个包对象 scala, 他是 com.sjyttkl.scala2这个包对应的包对象
  //3. 每一个包都可以有一个包对象
  //4. 包对象的名字需要和子包一样
  //5. 在包对象中可以定义变量，方法
  ...
  package scala2 {
    ...
    class Person { // 表示在 com.sjyttkl.scala下创建类 Person
      val name = "Nick"
